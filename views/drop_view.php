...
<html>
    <head>
        ...
        <h1 align="center">Eliminar Vista
            <h3>Vista DDL</h3>
        </h1>
    ...
    <body>
        <form method="get" action="http://localhost:8080/ADMIN/views/drop_view.php/?usuario=<?php echo $usuario;?>&
...
<?php 
    if(isset($_GET["ejecutar"])) {
        ...
        if(!$result) {
            echo "<script>alert('No se ha podido eliminar la Vista..!')</script>";
            echo    odbc_error($conexion->connection).": ".odbc_errormsg($conexion->connection);
        } else {
            echo "<script>alert('Se ha eliminado la vista..!')</script>";
        }   
    }
?>
