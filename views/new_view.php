...
<html>
    ...
    <body>
        <form method="get" action="http://localhost:8080/ADMIN/views/new_view.php/?usuario=<?php echo $usuario;?>&
...
<?php 
    if(isset($_GET["ejecutar"])) {
        ...
        if(!$result) {
            echo "<script>alert('No se ha podido crear la Vista..!')</script>";
            echo    odbc_error($conexion->connection).": ".odbc_errormsg($conexion->connection);
        } else {
            echo "<script>alert('Se ha creado la nueva Vista..!')</script>";
        }   
    }
?>
