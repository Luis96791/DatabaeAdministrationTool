 ...
 */
    ...
    $script->newTriggerDDL();   
    
    $_sql = "";
?>
<html>
    <head>
        <title>Nuevo Trigger</title>
        <h1 align="center">Crear Trigger
            <h3>Vista DDL</h3>
        </h1>
    ...
    <body>
        <form method="get" action="http://localhost:8080/ADMIN/triggers/new_trigger.php/?usuario=<?php echo $usuario;?>&
...
<?php 
    if(isset($_GET["ejecutar"])) {
        ...
        if(!$result) {
            echo "<script>alert('No se ha podido crear el Trigger..!')</script>";
            echo    odbc_error($conexion->connection).": ".odbc_errormsg($conexion->connection);
        } else {
            echo "<script>alert('Se ha creado el nuevo Trigger..!')</script>";
        }   
    }
?>