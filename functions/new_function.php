...
<html>
    ...
    <body>
        <form method="get" action="http://localhost:8080/ADMIN/functions/new_function.php/?usuario=<?php echo $usuario;?>&
...
<?php 
    if(isset($_GET["ejecutar"])) {
        ...
        if(!$result) {
            echo "<script>alert('No se ha podido crear la Funcion..!')</script>";
            echo    odbc_error($conexion->connection).": ".odbc_errormsg($conexion->connection);
        } else {
            echo "<script>alert('Se ha creado la nueva Funcion..!')</script>";
        }   
    }
?>